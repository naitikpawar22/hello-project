...
def get_result(rid,actor):
    ...
    if actor["role"]!="admin" and get_db().execute("SELECT creator_id FROM exams WHERE id=?",(r["exam_id"],)).fetchone()["creator_id"]!=actor["id"]: return jsonify(error="Not authorized"),403
...
def pdf(rid,actor):
    ...
    if not r: return jsonify(error="Result not found"),404
    if actor["role"]!="admin" and get_db().execute("SELECT creator_id FROM exams WHERE id=?",(r["exam_id"],)).fetchone()["creator_id"]!=actor["id"]: return jsonify(error="Not authorized"),403
    path=Path(current_app.config["UPLOAD_DIR"])/f"result_{rid}.pdf"; generate_result_pdf(r,path); return send_file(path,as_attachment=True,download_name=f"ExamForge_Result_{rid}.pdf",mimetype="application/pdf")
@bp.get("/public/<rid>")
def public_result(rid):
    r=result_detail(rid)
    if not r: return jsonify(error="Result not found"),404
    d={k:v for k,v in r.items() if k not in {"details","topic_performance","attempt_id"}}; return jsonify(result=d)
