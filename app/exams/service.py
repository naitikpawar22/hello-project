...
def get_exam(eid):
    ...
    for q in questions:
        q_dict = dict(q)
        opts_rows = db.execute(
            """
            SELECT id, option_index, option_text, is_correct
        ...
        opts = [dict(opt) for opt in opts_rows]
        correct_list = [o["option_index"] for o in opts if o.get("is_correct") == 1]
        q_dict["options"] = opts
        q_dict["correct_options"] = correct_list
