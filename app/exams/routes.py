...
def list_exams(actor):
    db = get_db()
    rows = db.execute("SELECT e.*,COUNT(eq.id) question_count FROM exams e LEFT JOIN exam_questions eq ON eq.exam_id=e.id GROUP BY e.id ORDER BY e.created_at DESC").fetchall()
    if actor["role"] != "admin":
        rows = [r for r in rows if r["creator_id"] == actor["id"]]
    exams = []
    for r in rows:
        d = dict(r)
...
def edit_exam(eid, actor):
    try:
        update_exam(eid, request.get_json(silent=True) or {}, actor)
        ...
        return jsonify(error=str(e)), 422
    audit(actor, "exam_updated", "exams", eid)
...
def publish(eid, actor):
    ...
    if actor["role"] != "admin" and e["creator_id"] != actor["id"]:
        return jsonify(error="Not authorized"), 403
    if db.execute("SELECT 1 FROM exam_questions WHERE exam_id=? LIMIT 1", (eid,)).fetchone() is None:
        return jsonify(error="Exam must contain at least one question"), 422
    status = "PUBLISHED"
    sched = db.execute("SELECT start_at,end_at FROM exam_schedules WHERE exam_id=?", (eid,)).fetchone()
    if sched:
        status = "SCHEDULED"
    db.execute("UPDATE exams SET status=?,updated_at=? WHERE id=?", (status, now_iso(), eid))
    db.commit()
    audit(actor, "exam_published", "exams", eid)
...
def blank_exam(actor):
    ...
    if not questions:
        return jsonify(error="At least one question is required"), 422
    normalized = []
    for q in questions:
        ...
        if not (2 <= len(opts) <= 4):
            return jsonify(error="Each question needs 2-4 options"), 422
        if not correct or (typ == "MCQ" and len(correct) != 1):
            return jsonify(error=f"Invalid correct answer for question: '{q.get('text','')}'"), 422
        normalized.append({
            ...
            "negative_marks": float(q.get("negative", 0) or 0),
            "topic": q.get("topic", "") or "",
            "explanation": q.get("explanation", "") or "",
            "needs_review": 0
        })
    if any(not q["text"] or q["marks"] < 0 or q["negative_marks"] < 0 for q in normalized):
        return jsonify(error="Question text and non-negative marks are required"), 422
