...
def schedule(eid, actor):
    try:
        schedule_exam(eid, request.get_json(silent=True) or {}, actor)
    ...
    audit(actor, "exam_scheduled", "exams", eid)
    return jsonify(message="Exam scheduled", schedule=exam_window(eid))

@bp.put("/<eid>/schedule")
@admin_or_teacher_required
def reschedule(eid, actor):
    return schedule(eid, actor)

@bp.get("/<eid>/schedule")
@admin_or_teacher_required
def schedule_status(eid, actor):
    s = exam_window(eid)
    return jsonify(schedule=s), 200
